...
def grahamScan(points):
    ...
    # 제외한 시작점을 맨앞에 이어붙이기 + 튜플 형태로 저장된 (좌표, 각) 에서 각 제거
    sorted_point = [start_point] + [p[0] for p in sorted(angle_rank, key=lambda x:x[1])] # 각도의 오름차순으로 정렬하되, 각도는 버리고 리스트에 좌표만 저장
